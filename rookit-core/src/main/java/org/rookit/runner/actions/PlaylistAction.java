...
import org.rookit.dm.play.TrackQueryDynamicPlaylistImpl;
...
public class PlaylistAction extends AbstractCommand {
    ...
    public PlaylistAction(RookitPlayer player, DBManager database) {
        super(OptionUtils.createArguments(values()),
                "Use this command to manage playlist");
    ...
    private void play() {
        if(current != null) {
            final Optional<Track> track = current.streamTracks()
                    .filter(t -> !t.getAudioContent().isEmpty())
                    .skip(trackIndex)
                    .findFirst();
            if(track.isPresent()) {
    ...
    private void select(String value) {
        final Playlist playlist = database.getPlaylists()
                ...
                .first();
        if(playlist == null) {
            println("No playlist found with official: " + value);
        }
        else {
            current = playlist;
            if(current instanceof TrackQueryDynamicPlaylistImpl) {
                ((TrackQueryDynamicPlaylistImpl) current).setDatabase(database);
            }
        }
    }
    ...
    private void create(String value) {
        // TODO this is ugly
        final TrackQueryDynamicPlaylistImpl playlist = new TrackQueryDynamicPlaylistImpl(value);

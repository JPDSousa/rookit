...
public class LastFMFactory extends AbstractModelFactory<de.umass.lastfm.Artist, de.umass.lastfm.Album, de.umass.lastfm.Track> {
	...
	public LastFMFactory(final RookitFactories factories,
			final MusicServiceConfig config, 
			final int levenshteinThreshold) {
		super(factories, config, levenshteinThreshold);
	}
	...
	public Track toTrack(de.umass.lastfm.Track source) {
		...
		final Document lastFM = new Document(ID, source.id())
		...
		final AlbumBuilder builder = parseTrackTitle(source.getName())
	...
	public Album toAlbum(de.umass.lastfm.Album source) {
		...
		if(tracks != null) {
			tracks.forEach(track -> album.addTrack(toTrack(track), track.getPosition(), Album.DEFAULT_DISC));
		}
		return album;
	}
	...
	public Set<Artist> toArtist(de.umass.lastfm.Artist source) {
		return factories.getArtistFactory().getArtistsFromFormat(source.getName());
	}
